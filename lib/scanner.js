 ...
 */
 ...
 class Scanner {
   ...
   async request(session, uuid, taglist) {
   ...
   scanning() {
     ...
     this.tagList.tags.forEach(tag => {
       if (tag.type && tag.type.structure && tag.type.typeName !== 'STRING') {
         const branch = { parentId, id: tag.id, title: tag.name };
         this.scanArray.push(branch);
         this.fromTemplate(tag.id, tag.name, tag.type.code, tag.program);
       } else {
         if (tag.type.typeName) {
          const leaf = this.getLeaf(parentId, tag.id, tag.name, tag.program, tag.type.typeName, tag.type.arrayDims);
   ...
   fromTemplate(parentId, parentName, code, program) {
     ...
     for (let i = 0; i < members.length; i++) {
       ...
       try {
         ...
         this.plugin.log(' item.type.structure ' + util.inspect(item) + util.inspect(e));
       }
     }
   }
